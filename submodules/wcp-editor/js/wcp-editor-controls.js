...
;(function ($, window, document, undefined) {
    ...
    $.wcpEditorRegisterControl({
        ...
        getValue: function() {
            var str = $('#wcpEditorTextarea-' + this.elementID).val();
            return str;
        },
        setValue: function(v) {
            // console.log(v);
            $('#' + this.elementID).val(v);
            $('#wcpEditorShowTextarea-' + this.elementID).html(v);
            $('#wcpEditorTextarea-' + this.elementID).html(v);
        },
        HTML: function(v) {
            var html = ''
        ...
        init: function() {
            ...
            $(document).on('click', '#savesummer-' + this.elementID, function() {
                $('#editsummer-' + self.elementID).css('background','#7757ce')
                $('#savesummer-' + self.elementID).css('background','#a3adb5')
                var markup = $('#wcpEditorTextarea-' + self.elementID).summernote('code');
                self.valueChanged();
                $('#wcpEditorTextarea-' + self.elementID).summernote('destroy');
                $('#wcpEditorTextarea-' + self.elementID).css('display','none');
                $('#wcpEditorShowTextarea-' + self.elementID).show();
                $('#wcpEditorShowTextarea-' + self.elementID).html(markup);
            });
            $(document).on('change', '#' + this.elementID, function() {
                self.valueChanged();
            });
    ...
    /*-- Select Police Value Need to be modified to show every Police, use specific controls with <li> and <span> not with select --*/
    $.wcpEditorRegisterControl({
        type: 'police',
        getValue: function() {
            return $('#' + this.elementID).val();
    ...
    $.wcpEditorRegisterControl({
        ...
        HTML: function() {
            return '<div id="'+ this.elementID +'" class="wcp-editor-control-button-re">'+ this.title +'</div>';
        },
        init: function() {
            var self = this;
    ...
    $.wcpEditorRegisterControl({
        ...
        HTML: function() {
            return '<div class="wcp-editor-input-with-button" id="'+ this.elementID +'"><input type="text"><div class="wcp-editor-control-button">Choisir</div></div>';
        },
        init: function() {
            var self = this;
    ...
    $.wcpEditorRegisterControl({
       ...
       //     $('#' + this.elementID + ' input').val(v);
        ...
        HTML: function() {
            return '<div class="wcp-editor-input-with-button" id="'+ this.elementID +'"><input type="file"></div>'; //<div class="wcp-editor-control-button">Image</div>
        },
        init: function() {
            var self = this;
    ...
    /*-- Controls for fullscreen position --*/
    $.wcpEditorRegisterControl({
