;(function ($, window, document, undefined) {
	...
	var emailUser = undefined;
	var quizzFromUser = undefined;
	...
	var EDITOR_ONE_GEO_ONLY = true;
	var EDITOR_ONE_INFOS_LEGALE_ONLY = true;
	...
	var EDITOR_TOOL_GEO = 'geo';
	var EDITOR_TOOL_INFOS_LEGALE = 'infoslegale';
	...
	// WCP INFORMATION USERS API ==========================================================
	$.wcpGetEmailUser = function(){
		return("TEST@EMAIL"); // need method to recup the email from Users to get all quizz from users
	}
	$.wcpCallbackFromServer = function(data){
		allquizz = data;
	}
	$.wcpGetQuizzAllByUser = function(email, url, callback){
		...
		console.log(email);
		xmlHttp.open("GET", url, true); // true for asynchronous and need to write email
		xmlHttp.send(null);
	}
	// WCP EDITOR API ==========================================================
	...
	$.wcpEditorGetListItems = function() {
		var items = [];
		// console.log(settings); GetList Items for wpc-right
		// Returns an array of objects in the format { id: 'id', title: 'title' }
		for (var i=settings.spots.length - 1; i>=0; i--) {
			var s = settings.spots[i];
			if(s.id === "geolocation" ){
				EDITOR_ONE_GEO_ONLY = false; //Only one spot geolocation
			}
			if (s.id === "infoslegale" ){
				EDITOR_ONE_INFOS_LEGALE_ONLY = false; // Only one spot infoslegale
			}
	...
	$.getImagefromField = function(input_file){
		...
		var url = $(input_file).val();
		var ext = url.substring(url.lastIndexOf('.') + 1).toLowerCase();
		if (input.files && input.files[0]&& (ext == "gif" || ext == "png" || ext == "jpeg" || ext == "jpg")) {
			var m = $.wcpEditorGetModelOfFormWithName("Image Map Settings");
			...
			reader.onload = function (e) {
				...
				image.onload = function() {
					...
					settings.general.width = this.width;
					settings.general.height = this.height
					editor.canvas.width = this.width;
					editor.canvas.height = this.height;
					settings.general.naturalWidth = this.width;
					settings.general.naturalHeight = this.height;
				
				};
				var imageData = e.target.result;
				
				editor.canvasImage.src = e.target.result;

				settings.image.url = e.target.result;
				settings.general.image_url = e.target.result;
			...
			/*	settings.general.width = og_width
				settings.general.height = og_height;
				editor.updateImageMapForm();
				editor.redraw();
				*/
			//  $('#imp-editor-image').attr('src', e.target.result);
			}
			...
			setTimeout(function(){
				...
				$('.wcp-editor-control-button-re').trigger('click');
			},100);
		} else {
	...
	$.wcpEditorEventFormUpdated = function(formName, controlName) {
		if (formName == 'Image Map Settings') {
			...
			//Don't forget 'm' is the list of image-map-pro-editor-init-jquery so some name are different of defaultsettings becarfull
			...
			if (m.image.image_url && m.image.image_url.length > 0 && settings.image.url !== m.image.image_url) {
				...
				settings.general.background_image_color = m.image.background_image;
				...
				settings.general.fullname = m.general.fullname;
				settings.general.savoirplus = m.general.savoirplus;
				...
				// add settings title 
				if (m.title) {
					settings.title.title_name = m.title.title_name;
					settings.title.title_align = m.title.title_align;
					settings.title.title_color = m.title.title_color;
					settings.title.title_color_sep = m.title.title_color_sep;
					settings.title.title_fontsize = m.title.title_fontsize;
					settings.title.title_police = m.title.title_police;
				}
				//add settings for pharase d'accroche
				if(m.phase){
					settings.phase.phase_accroche = m.phase.phase_accroche;
					settings.phase.phase_descriptif = m.phase.phase_descriptif;
					settings.phase.phase_contour_color = m.phase.phase_contour_color;
					settings.phase.phase_fond_color = m.phase.phase_fond_color;
					settings.phase.phase_text_color = m.phase.phase_text_color;
					settings.phase.phase_popover_exist = m.phase.phase_popover_exist;
					settings.phase.phase_header_exist = m.phase.phase_header_exist;
				}
				if(m.infosplus){
					settings.infosplus.infosplus_title = m.infosplus.infosplus_title;
					settings.infosplus.infosplus_descriptif = m.infosplus.infosplus_descriptif;
					settings.infosplus.infosplus_contour_color = m.infosplus.infosplus_contour_color;
					settings.infosplus.infosplus_fond_color = m.infosplus.infosplus_fond_color;
					settings.infosplus.infosplus_text_color = m.infosplus.infosplus_text_color;
					settings.infosplus.infosplus_exist = m.infosplus.infosplus_exist;
				}
				if(m.fullscreen) {
		...
		if (formName == 'Shape Settings' && editor.selectedSpot !== undefined) {
			...
			//Don't forget 'model' is the list of image-map-pro-editor-init-jquery so some name are different of defaultsettings becarfull
			console.log("FORM SHAPE NAME---------------",$.wcpEditorGetModelOfFormWithName('Shape Settings'));
			...
			s.default_style.use_icon_style = model.icon.use_icon_style;
	...
	$.wcpEditorEventNewButtonPressed = function() {
		EDITOR_ONE_GEO_ONLY = true;
		EDITOR_ONE_INFOS_LEGALE_ONLY = true;
	}
	$.wcpEditorEventSaveButtonPressed = function() {
		$.wcpEditorPresentLoadingScreen('Enregistrement...');

		$.imp_editor_storage_store_save(editor.getCompressedSettings(), function(success) {
			if (success) {
				$.wcpEditorHideLoadingScreenWithMessage('Enregistré !', false, false);
			} else {
				$.wcpEditorHideLoadingScreenWithMessage('Une erreur s\'est produite lors de la Mini Expo', true, false);

				var modalBody = '';

				modalBody += '<div class="modal-info-text">Veuillez sauvegarder ce code afin de préserver votre travail et réessayer plus tard. <br>Ce code peut être importé à tout moment en ouvrant une carte-image existante et en appuyant sur le bouton Importer <br>Vous pouvez aussi <a href="https://www.culturmoov.com/contact">Contacter nous</a>!</div>';
				modalBody += '<textarea id="textarea-error-saving">'+ JSON.stringify(editor.getCompressedSettings()) +'</textarea>';

				setTimeout(function() {
					$.wcpEditorPresentModal({
						name: 'error-saving',
						title: 'Erreur lors de l\'enregistrement de la Mini Expo',
						buttons: [
							{
								class: 'primary',
								name: 'done',
	...
	$.wcpEditorEventLoadButtonPressed = function() {
		// Write if GEOLOCATION AND INFOLEGALES VAR IS AVAILBLE SET if not Pass TRUE
		$("#firstImage").hide();	
	}
	...
	$.wcpEditorEventListItemTitleButtonPressed = function(buttonName) {
        ...
        if (buttonName == 'duplicate') {
			...
			if(s.type == 'geo'|| s.type == 'infoslegale') {
				$.wcpEditorPresentModal({
					name: 'informations',
					title: 'Infos',
					buttons: [
					...
					body: 'Un seul bouton de ce type uniquement'
				});
				return;
			}
		...
		if (buttonName == 'rename') {
			...
			if (s.type == 'geo' || s.type == 'infoslegale'){
				$.wcpEditorPresentModal({
					...
					body: 'Impossible de renommer ce bouton'
				});
				return;
			}			
			var html = '<div class="wcp-editor-form-control">';
			html += '<label>Nom de la forme</label>';
	...
	$.wcpEditorEventSelectedTool = function(toolName) {
		console.log(toolName, "EventSelectedTool");
		settings.editor.tool = toolName;
		// for (var g = 0; g < settings.spots.length; g++){
		// 	settings.spots[g].type === "infoslegale" ? EDITOR_ONE_INFOS_LEGALE_ONLY = true : EDITOR_ONE_INFOS_LEGALE_ONLY = false;
		// 	settings.spots[g].type === "geo" ? EDITOR_ONE_GEO_ONLY = true : EDITOR_ONE_GEO_ONLY = false;
		// }
		editor.redraw();
	}
	...
	$.wcpEditorEventExtraMainButtonClick = function(buttonName) {
		if (buttonName == 'code') {
			...
			html += '    Ceci est un exemple de document HTML, montrant comment installer le plugin dans votre site web.';
			html += '</div>';
			...
			html += '</pre>';
			html += '<div class="generated-code-help">Le code qui contient tous les paramètres et initialise le plugin :</div>';
	...
	$.wcpEditorEventModalButtonClicked = function(modalName, buttonName) {
		...
		if (modalName == 'confirm-delete-shape') {
			if (buttonName == 'delete') {
				...
				if (settings.editor.selected_shape == settings.spots[indexOfShapeToDelete].id) {
					if (editor.selectedSpot.id == "geolocation") {
						EDITOR_ONE_GEO_ONLY = true;
					}
					if (editor.selectedSpot.id == "infoslegale") {
						EDITOR_ONE_INFOS_LEGALE_ONLY = true;
					}
					editor.deselectSpot();
				}
		...
		if (modalName == 'confirm-rename-shape') {
			...
			if (shapeTitleExists || newTitle.length == 0) {
				$('#rename-shape-error').show().html('Une forme avec ce nom existe déjà !');
				return;
			}
	...
	$.wcpEditorEventCreatedNewInstance = function(instanceName) {
		...
		$.wcpEditorPresentLoadingScreen('Création ...');
		...
		$.imp_editor_storage_store_save(editor.getCompressedSettings(), function() {
			$.imp_editor_storage_set_last_save(settings.id, function() {
				...
				$.wcpEditorHideLoadingScreenWithMessage('Créé !', false, false);
			});
		});
	}
	...
	$.wcpEditorEventLoadSaveWithID = function(saveID) {
		$.wcpEditorPresentLoadingScreen('Chargement ...');

		$.imp_editor_storage_get_save(saveID, function(save) {
			if (!save) {
				$.wcpEditorHideLoadingScreenWithMessage('Erreur lors du chargement.', true, false);
			} else {
				settings = save;
	...
	$.image_map_pro_editor_current_options = function() {
		return $.WCPEditorSettings.listItemTitleButtons;
	...
	$.image_map_pro_editor_spots = function() {
	...
	function Editor() {
		...
		this.startedDrawingGeo = false;
		this.startedDrawingInfosLegale = false;
	...
	Editor.prototype.init = function(initSettings, wcpEditorSettings) {
		...
		if (initSettings) {
			...
			console.log(settings, "SETTINGS IS");
			// launch
			self.launch();
		} else {
			// Load last save
				...
				/*	if (!$.wcpTourIsFinished('Image Map Pro Editor Tour')) {
						...
						$.imp_editor_storage_get_saves_list(function(savesList) {
							if (savesList.length > 0) {
								console.log("Somes Saves");
								// Launch with defaults
								settings = $.extend(true, {}, default_settings);
								settings.general.name = 'Untitled';
								settings.id = uniqid()//Math.round(Math.random() * 10000) + 1;
								self.launch();
	...
	Editor.prototype.launch = function() {
		...
		if (settings.image.url && settings.image.url.length > 0) {
			...
			loadImage(self.canvasImage, function() {
				...
				$.wcpEditorHideLoadingScreenWithMessage('Erreur de chargement de l\'image!', true, false);
			});
		} else {
			$.wcpEditorHideLoadingScreen();
			$.wcpEditorOpenMainTabWithName('Image Map');
	...
	Editor.prototype.parseSettings = function() {
		...
		settings.title = $.extend(true, {}, default_settings.title, settings.title);
		settings.phase = $.extend(true, {}, default_settings.phase, settings.phase);
		settings.infosplus = $.extend(true, {}, default_settings.infosplus, settings.infosplus);
		...
		//update title name
		if(settings.general.title_name){
			settings.title.title_name = settings.general.title_name;
			settings.general.title_name = undefined;
		}
		if(settings.general.title_police){
			settings.title.title_police = settings.general.title_police;
			settings.general.title_police = undefined;
		}
		if(settings.general.title_fontsize){
			settings.title.title_fontsize = settings.general.title_fontsize;
			settings.general.title_fontsize = undefined;
		}
		if(settings.general.title_color){
			settings.title.title_color = settings.general.title_color;
			settings.general.title_color = undefined;
		}
		if(settings.general.title_align){
			settings.title.title_align = settings.general.title_align;
			settings.general.title_align = undefined;
		}
		// update phrase d'accroche
		if(settings.general.phase_accroche){
			settings.phase.phase_accroche = settings.general.phase_accroche;
			settings.general.phase_accroche = undefined;
		}
		if(settings.general.phase_descriptif){
			settings.phase.phase_descriptif = settings.general.phase_descriptif;
			settings.general.phase_descriptif = undefined;

		}
		if(settings.general.phase_contour_color){
			settings.phase.phase_contour_color = settings.general.phase_contour_color;
			settings.general.phase_contour_color = undefined;

		}
		if(settings.general.phase_fond_color){
			settings.phase.phase_fond_color = settings.general.phase_fond_color;
			settings.general.phase_fond_color = undefined;

		}
		if(settings.general.phase_text_color){
			settings.phase.phase_text_color = settings.general.phase_text_color;
			settings.general.phase_text_color = undefined;

		}
		if(settings.general.phase_popover_exist){
			settings.phase.phase_popover_exist = settings.general.phase_popover_exist;
			settings.general.phase_popover_exist = undefined;
		}
		if(settings.general.phase_header_exist){
			settings.phase.phase_header_exist = settings.general.phase_header_exist;
			settings.general.phase_header_exist = undefined;
		}
		
		// update infosplus d'accroche
		if(settings.general.infosplus_title){
			settings.infosplus.infosplus_title = settings.general.infosplus_title;
			settings.general.infosplus_title = undefined;
		}
		if(settings.general.infosplus_descriptif){
			settings.infosplus.infosplus_descriptif = settings.general.infosplus_descriptif;
			settings.general.infosplus_descriptif = undefined;

		}
		if(settings.general.infosplus_contour_color){
			settings.infosplus.infosplus_contour_color = settings.general.infosplus_contour_color;
			settings.general.infosplus_contour_color = undefined;

		}
		if(settings.general.infosplus_fond_color){
			settings.infosplus.infosplus_fond_color = settings.general.infosplus_fond_color;
			settings.general.infosplus_fond_color = undefined;

		}
		if(settings.general.infosplus_text_color){
			settings.infosplus.infosplus_text_color = settings.general.infosplus_text_color;
			settings.general.infosplus_text_color = undefined;

		}
		if(settings.general.infosplus_exist){
			settings.infosplus.infosplus_exist = settings.general.infosplus_exist;
			settings.general.infosplus_exist = undefined;
		}
		...
		settings.general.fullname = settings.general.fullname.trim();
		settings.general.savoirplus = settings.general.savoirplus;
		settings.general.background_image_color = settings.general.background_image_color;
		console.log(settings.general.background_image_color, "line 1634");
		settings.general.shortcode = settings.general.shortcode.trim();
		
		
		settings.general.title_name = settings.general.title_name;
		settings.general.title_police = settings.general.title_police;
		settings.general.title_fontsize = settings.general.title_fontsize;
		settings.general.title_color = settings.general.title_color;
		settings.general.title_align = settings.general.title_align;
		settings.general.title_color_sep = settings.general.title_color_sep;

		
		settings.general.phase_accroche = settings.general.phase_accroche;
		settings.general.phase_descriptif = settings.general.phase_descriptif;
		settings.general.phase_fond_color = settings.general.phase_fond_color;
		settings.general.phase_text_color = settings.general.phase_text_color;
		settings.general.phase_contour_color = settings.general.phase_contour_color;
		settings.general.phase_popover_exist = settings.general.phase_popover_exist;
		settings.general.phase_header_exist = settings.general.phase_header_exist;

		settings.general.infosplus_title = settings.general.infosplus_title;
		settings.general.infosplus_descriptif = settings.general.infosplus_descriptif;
		settings.general.infosplus_fond_color = settings.general.infosplus_fond_color;
		settings.general.infosplus_text_color = settings.general.infosplus_text_color;
		settings.general.infosplus_contour_color = settings.general.infosplus_contour_color;
		settings.general.infosplus_exist = settings.general.infosplus_exist;
	}
	Editor.prototype.redraw = function() {
		...
		var spots = $.image_map_pro_editor_spots();
		this.allSpots = $.extend(true, {}, spots);
		...
		if (parseInt(settings.editor.previewMode, 10) == 0) {
			...
			//Html the title
			$('#wcp-editor-title-proj').html($.image_map_pro_editor_content_title());
			
			//Html the phase popover
	...
	Editor.prototype.getCanvasDefaultSize = function() {
		...
		// 	//console.log(self.canvasImage.naturalWidth);
	...
	Editor.prototype.getTitleValue = function() {
	...
	Editor.prototype.events = function() {
		var self = this;
		// $(document).off('click', '#wcp-editor-infosplus-btn-first');
		// $(document).on('click','#wcp-editor-infosplus-btn-first', function() {
		// 	console.log("COUCOU EVENT EDITOR");
		// });

		$(document).off('click', '#wcp-editor-infosplus-btn-sd');
		$(document).on('click','#wcp-editor-infosplus-btn-sd', function() {
			$(this).toggleClass('active');
			$('#wcp-editor-infosplus-btn-first').toggleClass('active');
		});
		...
		// this function is reponsibel for creating and updating size
		...
		$(document).off('click','.wcp-editor-control-button-launch');
		$(document).on('click','.wcp-editor-control-button-launch',function(){
			setTimeout(() => {
		...
		$(document).on('click','#extra-button-mobile',function(){
			settings.general.width = 320;
			settings.general.height = 528;
			self.updateImageMapForm();
			self.redraw();
		});
		$(document).on('click','#extra-button-tablet',function(){
			settings.general.width = self.canvasImage.naturalWidth;
			settings.general.height = self.canvasImage.naturalHeight;
			self.updateImageMapForm();
			self.redraw();
	...
	Editor.prototype.handleMouseDown = function(e) {
		...
		if (isEventInsideCanvas) {
			...
			if (settings.editor.tool == EDITOR_TOOL_GEO && EDITOR_ONE_GEO_ONLY) {
				self.startedDrawingGeo = true;
				EDITOR_ONE_GEO_ONLY = false;
				return;
			}
			if (settings.editor.tool == EDITOR_TOOL_INFOS_LEGALE && EDITOR_ONE_INFOS_LEGALE_ONLY) {
				self.startedDrawingInfosLegale = true;
				EDITOR_ONE_INFOS_LEGALE_ONLY = false;
				return;
			}
	...
	Editor.prototype.handleMouseUp = function(e) {
		...
		if (self.startedDrawingGeo) {
			var s = self.createGeo();
			s.x = 100
			s.y = 100 
		...
		if (self.startedDrawingInfosLegale) {
			var s = self.createInfos();
			s.x = 0
			s.y = 100 
		...
		self.startedDrawingGeo = false;
		self.startedDrawingInfosLegale = false;
	...
	Editor.prototype.createIdForGeo = function() {
		var id = 'geolocation'
		return id;
	}
	Editor.prototype.createIdForInfos = function() {
		var id = 'infoslegale'
		return id;
	}
	...
	Editor.prototype.createGeo = function() {
		var self = this;
	...
	Editor.prototype.createInfos = function() {
		var self = this;
	...
	Editor.prototype.zoomIn = function(e) {
		...
		if (self.zoom > editorMaxZoomLevel) {
			...
			self.canvasX += dx;
			self.canvasY += dy;
			// for (var spot = 0; spot < self.allSpots.length; spot++){
			// 	self.allSpots[spot].width *= 2;
			// 	self.allSpots[spot].height *= 2;
			// }
			// Redraw
			self.redrawCanvas();
			self.redraw();
	...
	Editor.prototype.zoomReset = function() {
		...
		var spots = $.image_map_pro_editor_spots();
		// console.log(this);
		// for (var spot = 0; spot < this.allSpots.length; spot++){
		// 	spots[spot].width = this.allSpots[spot].width;
		// 	spots[spot].height = this.allSpots[spot].height;;
		// }
		this.canvasX = 0;
		this.canvasY = 0;
	...
	Editor.prototype.updateShapesForm = function() {
		...
		if (s) {
			...
			$.wcpEditorSetControlValue('Shape Settings', 'use_icon_style', s.default_style.use_icon_style);
			...
			$(".wcp-editor-control-button-standalone").show();
			...
			$.wcpEditorSetContentForTabWithName('Shape', '<div class="w	cp-editor-form-tabs-content-wrap">Sélectionnez ou créez un point d\intêret</div>');
			$(".wcp-editor-control-button-standalone").hide();
	...
	Editor.prototype.updateShapesFormState = function() {
		...
		// Remove It to show the controls in shape
		$.wcpEditorFormHideControlsGroup('Shape Settings', 'tooltip_content'); // Contenus de l'infobulle
		$.wcpEditorFormHideControlsGroup('Shape Settings', 'mouseover_style'); // Style de la souris
		$.wcpEditorFormHideControlsGroup('Shape Settings', 'actions'); // Actions
		...
		if (s.type == 'spot' || s.type == 'geo' || s.type == 'infoslegale') {
			...
			if (parseInt(s.default_style.use_icon_style, 10) == 1) {
				$.wcpEditorFormShowControlsGroup('Shape Settings', 'default_style');
			}
			else {
			...
			if (parseInt(s.default_style.use_icon, 10) == 1) {
				...
				console.log("SHOW SETTINGS ICONE HERE");
	...
	Editor.prototype.updateImageMapForm = function() {
		...
		$.wcpEditorSetControlValue('Image Map Settings', 'fullname', settings.general.fullname);

		$.wcpEditorSetControlValue('Image Map Settings', 'background_image', settings.general.background_image_color);
		console.log(settings.general.background_image_color, "line 3935");
		...
		$.wcpEditorSetControlValue('Image Map Settings', 'savoirplus', settings.general.savoirplus);	
		...
		$.wcpEditorSetControlValue('Image Map Settings', 'title_name', settings.title.title_name);
		$.wcpEditorSetControlValue('Image Map Settings', 'title_police', settings.title.title_police);
		$.wcpEditorSetControlValue('Image Map Settings', 'title_color', settings.title.title_color);
		$.wcpEditorSetControlValue('Image Map Settings', 'title_color_sep', settings.title.title_color_sep);
		$.wcpEditorSetControlValue('Image Map Settings', 'title_fontsize', settings.title.title_fontsize);
		$.wcpEditorSetControlValue('Image Map Settings', 'title_align', settings.title.title_align);

		// add phrase d'accroche
		$.wcpEditorSetControlValue('Image Map Settings', 'phase_accroche', settings.phase.phase_accroche);
		$.wcpEditorSetControlValue('Image Map Settings', 'phase_descriptif', settings.phase.phase_descriptif);
		$.wcpEditorSetControlValue('Image Map Settings', 'phase_fond_color', settings.phase.phase_fond_color);
		$.wcpEditorSetControlValue('Image Map Settings', 'phase_contour_color', settings.phase.phase_contour_color);
		$.wcpEditorSetControlValue('Image Map Settings', 'phase_text_color', settings.phase.phase_text_color);
		$.wcpEditorSetControlValue('Image Map Settings', 'phase_popover_exist', settings.phase.phase_popover_exist);
		$.wcpEditorSetControlValue('Image Map Settings', 'phase_header_exist', settings.phase.phase_header_exist);

		// console.log(settings);
		// add infosplus
		$.wcpEditorSetControlValue('Image Map Settings', 'infosplus_exist', settings.infosplus.infosplus_exist);
		$.wcpEditorSetControlValue('Image Map Settings', 'infosplus_title', settings.infosplus.infosplus_title);
		$.wcpEditorSetControlValue('Image Map Settings', 'infosplus_descriptif', settings.infosplus.infosplus_descriptif);
		$.wcpEditorSetControlValue('Image Map Settings', 'infosplus_fond_color', settings.infosplus.infosplus_fond_color);
		$.wcpEditorSetControlValue('Image Map Settings', 'infosplus_contour_color', settings.infosplus.infosplus_contour_color);
		$.wcpEditorSetControlValue('Image Map Settings', 'infosplus_text_color', settings.infosplus.infosplus_text_color);
	...
	Editor.prototype.updateImageMapFormState = function() {
		// Show/hide controls
		// Remove It to show the controls in Image Map Setting
		
		$.wcpEditorFormHideControlsGroup('Image Map Settings', 'tooltips'); // Parametre de l'infos bulle
		$.wcpEditorFormHideControlsGroup('Image Map Settings', 'fullscreen'); // Parametre de l'infos bulle
		// Actif or not infosplus  exist
		if (parseInt(settings.infosplus.infosplus_exist, 10) == 1) {
			$.wcpEditorFormShowControl('Image Map Settings', 'infosplus_title');
			$.wcpEditorFormShowControl('Image Map Settings', 'infosplus_descriptif');
			$.wcpEditorFormShowControl('Image Map Settings', 'infosplus_fond_color');
			$.wcpEditorFormShowControl('Image Map Settings', 'infosplus_contour_color');
			$.wcpEditorFormShowControl('Image Map Settings', 'infosplus_text_color');
		} else {
			$.wcpEditorFormHideControl('Image Map Settings', 'infosplus_title');
			$.wcpEditorFormHideControl('Image Map Settings', 'infosplus_descriptif');
			$.wcpEditorFormHideControl('Image Map Settings', 'infosplus_fond_color');
			$.wcpEditorFormHideControl('Image Map Settings', 'infosplus_contour_color');
			$.wcpEditorFormHideControl('Image Map Settings', 'infosplus_text_color');
		}
		
		// Actif or not phase footer exist
		if (parseInt(settings.phase.phase_popover_exist, 10) == 1) {
			$.wcpEditorFormShowControl('Image Map Settings', 'phase_accroche');
			$.wcpEditorFormShowControl('Image Map Settings', 'phase_descriptif');
			$.wcpEditorFormShowControl('Image Map Settings', 'phase_fond_color');
			$.wcpEditorFormShowControl('Image Map Settings', 'phase_contour_color');
			$.wcpEditorFormShowControl('Image Map Settings', 'phase_text_color');
		} else {
			$.wcpEditorFormHideControl('Image Map Settings', 'phase_accroche');
			$.wcpEditorFormHideControl('Image Map Settings', 'phase_descriptif');
			$.wcpEditorFormHideControl('Image Map Settings', 'phase_fond_color');
			$.wcpEditorFormHideControl('Image Map Settings', 'phase_contour_color');
			$.wcpEditorFormHideControl('Image Map Settings', 'phase_text_color');
		}
	...
	Editor.prototype.launchTooltipContentBuilder = function() {
		if ($('#imp-editor-tooltip-content-builder-wrap').length == 0) {
			...
			html += '       <p>Appuyez sur le bouton Terminé lorsque vous avez terminé l\'édition <br>ou cliquez sur le bouton Fermer dans le coin supérieur droit.</p>';
			html += '   </div>';
			html += '   <div class="wcp-editor-control-button" id="imp-editor-done-editing-tooltip">Terminé</div>';
			html += '</div>';
		...
		$.squaresInitWithSettings($('#imp-editor-tooltip-content-builder'), this.selectedSpot.tooltip_content.squares_settings, this.selectedSpot.id);
	...
	Editor.prototype.getCompressedSettings = function() {
		...
		console.log(compressed,"FOR EXPORT");
		var compressedSpots = [];
