    ...
    $.extend(Plugin.prototype, {
                ...
                if (s.type == 'spot' || s.type == 'geo' || s.type == 'infoslegale') {
                    if (parseInt(s.default_style.use_icon, 10) == 1) {
                        ...
                        if (s.default_style.icon_type == 'library') {
                            // Custom icon if use style icon
                            if (parseInt(s.default_style.use_icon_style, 10) == 1) {
                                var css = this.cssStyle(s.default_style, i);
                            }
                            html += '   <svg style="' + css +'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" viewBox="'+ s.default_style.icon_svg_viewbox +'" xml:space="preserve" width="'+ s.width +'px" height="'+ s.height +'px">';
                ...
                for (var i=0; i<self.settings.spots.length; i++) {
                    ...
                    if (s.tooltip_content.content_type == 'plain-text') {
						...
						if (s.tooltip_content.squares_json) {
                            console.log("squares_json");
							html += $.squaresRendererRenderObject(s.tooltip_content.squares_json, s.id);
						} else {
                            console.log("squares_settings");
							html += $.squaresRendererRenderObject(s.tooltip_content.squares_settings, s.id);
						}
                    }
                ...
                for (var i=0; i<self.settings.spots.length; i++) {
                    ...
                    if (s.tooltip_content.content_type == 'plain-text') {
						...
						if (s.tooltip_content.squares_json) {
                            ...
                            console.log("squares_settings Other always");
							html += $.squaresRendererRenderObject(s.tooltip_content.squares_settings, s.id);
						}
                    }
        ...
        initFullscreen: function() {
            ...
            if (parseInt(self.settings.fullscreen.enable_fullscreen_mode, 10) == 1) {
                ...
                var text = 'Ouvrir en plein écran';
                if (parseInt(self.settings.runtime.is_fullscreen, 10) == 1) {
        ...
        events: function() {
            ...
            //Event to check click quizz only function of settings.id
            $(document).off('click.' + this.settings.id, '.quizz-button');
            $(document).on('click.' + this.settings.id, '.quizz-button', function() {
                var quizzId = $(this).data().question, responseId = $(this).data().quizz, question = $('#question-' + quizzId), response = $('#' + responseId).html();
                $(this).data().resp ? ( question.html(response), $('#button-' + quizzId).hide(), $(this).css({backgroundColor: "green"})) : (question.html(response), $('#button-' + quizzId).removeClass('quizz-button-check'), $('#button-' + quizzId).show(), $(this).css({backgroundColor: "red"}));
                $(this).closest('.quizz-content').children().each((index, value) => {
                   $(value).hasClass('quizz-col') && ($(value).children().attr("disabled", true), $(value).children().css('opacity', 1));
                });
                $(this).css('opacity', 0.65)
            });

            $(document).off('click.' + this.settings.id, '.quizz-button-again');
            $(document).on('click.' + this.settings.id, '.quizz-button-again', function() {
                var quizzId, back_color;
                $(this).closest('.quizz-content').children().each((index, value) => {
                    $(value).hasClass('quizz-col') && ($(value).children().attr("disabled", false), quizzId = $(value).children().data().question, back_color = $(value).children().data().back, $(value).children().css({opacity: 1, backgroundColor: back_color}));
                });
                $('#question-' + quizzId).html($('#save-' + quizzId).html()), $(this).hide();
            });
        ...
        applyStyles: function(styles, i) {
            ...
            if ((s.type == 'spot' || s.type == 'geo' || s.type == 'infoslegale') && el.find('path').length > 0) {
        ...
        highlightShape: function(i, recursive) {
          ...
          //  console.log(this.settings.spots[i].mouseover_style);
        ...
        updateTooltipPosition: function(i, e) {
            ...
            if ((s.type == 'spot'|| s.type == 'geo' || s.type == 'infoslegale')
             && parseInt(s.default_style.icon_is_pin, 10) == 1 && (s.type == 'spot' || s.type == 'geo' || s.type == 'infoslegale')
            && parseInt(s.default_style.use_icon, 10) == 1) {
                y -= sh/2;
            }
        ...
        toggleFullscreen: function() {
            if (parseInt(this.settings.runtime.is_fullscreen, 10) == 0) {
                ...
                fullscreenSettings.general.savoirplus = 0;

                console.log(fullscreenSettings);
                var style = '';
                style += 'background: ' + this.settings.fullscreen.fullscreen_background;

                var html = '';
                html += '<div id="imp-fullscreen-wrap" style="'+ style +'">'
                html += '   <div id="image-map-pro-title-'+ fullscreenSettings.id +'">'
                html += '   </div>'
                html += '   <div id="image-map-pro-'+ fullscreenSettings.id +'">'
                html += '   </div>'
                html += '   <div id="image-map-pro-footer-'+ fullscreenSettings.id +'">'
                html += '   </div>'
                html += '</div>'

                $('body').append(html);
                console.log("Fullscreen active here");
